  ...
  <div class="ims-scope w-full mx-auto space-y-6">
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="font-bold text-2xl text-slate-900 tracking-tight flex items-center gap-2">
          ...
          Quáº£n lÃ½ Sinh viÃªn & Há»“ sÆ¡ Chi tiáº¿t
        </h1>
        <p class="text-xs text-slate-500 mt-1">Danh sÃ¡ch táº¥t cáº£ sinh viÃªn thuá»™c cÃ¡c lá»›p thá»±c táº­p do báº¡n trá»±c tiáº¿p hÆ°á»›ng dáº«n.</p>
      </div>

      <div class="flex items-center gap-3">
        <span class="bg-blue-50 text-blue-800 text-xs font-bold px-3 py-1.5 rounded-xl border border-blue-200">
          Tá»•ng sá»‘: {{ students.length }} sinh viÃªn
        </span>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
          <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Tá»•ng Sinh viÃªn</p>
          <p class="text-2xl font-black text-slate-900 mt-0.5">{{ students.length }}</p>
          <p class="text-[11px] text-slate-500 mt-0.5">Thuá»™c {{ classCount }} lá»›p hÆ°á»›ng dáº«n</p>
        </div>
      ...
      <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
          <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">ÄÃ£ cÃ³ Doanh nghiá»‡p</p>
      ...
      <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
          <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">ÄÃ£ ghÃ©p NhÃ³m</p>
          <p class="text-2xl font-black text-indigo-600 mt-0.5">{{ withGroupCount }}</p>
          <p class="text-[11px] text-indigo-600 font-semibold mt-0.5">NhÃ³m thá»±c táº­p tá»± quáº£n</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs space-y-4">
      <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        <div class="relative flex-1">
          ...
          <input
            v-model="search"
            placeholder="TÃ¬m kiáº¿m há» tÃªn, MSSV, lá»›p, doanh nghiá»‡p..."
        ...
        <table class="w-full text-left text-xs border-collapse">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase tracking-wider">
              <th class="p-4">Sinh viÃªn</th>
              <th class="p-4">Lá»›p thá»±c táº­p</th>
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-if="loading" class="text-center">
              <td colspan="6" class="p-8 text-slate-500">Äang táº£i danh sÃ¡ch sinh viÃªn...</td>
            </tr>
            <tr v-else-if="!filteredStudents.length" class="text-center">
              <td colspan="6" class="p-8 text-slate-500">KhÃ´ng tÃ¬m tháº¥y sinh viÃªn thá»a mÃ£n Ä‘iá»u kiá»‡n.</td>
            </tr>
            <tr v-for="s in filteredStudents" :key="s.maSinhVien" class="hover:bg-slate-50 transition-colors">
              ...
              <td class="p-4">
                ...
                >
                  {{ statusLabels[s.trangThaiThucTap] || 'Äang thá»±c táº­p' }}
                </span>
              </td>
    ...
    <!-- Drawer Há»“ sÆ¡ Chi tiáº¿t Sinh viÃªn -->
        ...
        <div class="p-6 overflow-y-auto space-y-6 flex-1 text-xs">
          ...
          <div class="space-y-3">
            <h4 class="font-bold text-slate-900 uppercase tracking-wider text-[11px] text-blue-900 border-b border-slate-100 pb-1">
              ThÃ´ng tin Há»c pháº§n & LiÃªn há»‡
            </h4>
            <div class="grid grid-cols-2 gap-3 text-slate-700">
              <div><span class="text-slate-400 block text-[10px] uppercase">Lá»›p sinh hoáº¡t:</span> <strong>{{ detail.lopSinhHoat || '22CNTT1' }}</strong></div>
              <div><span class="text-slate-400 block text-[10px] uppercase">Lá»›p thá»±c táº­p:</span> <strong>{{ detail.tenLop || 'MIS2012_01' }}</strong></div>
          ...
          <div class="space-y-3">
            <h4 class="font-bold text-slate-900 uppercase tracking-wider text-[11px] text-blue-900 border-b border-slate-100 pb-1">
              Doanh nghiá»‡p & CÃ¡n bá»™ hÆ°á»›ng dáº«n
            </h4>
            <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2.5">
              <div>
                <span class="text-slate-400 text-[10px] uppercase block font-bold">ÄÆ¡n vá»‹ thá»±c táº­p:</span>
              ...
              <div class="grid grid-cols-2 gap-2 pt-1 border-t border-slate-200/60">
                <div>
                  <span class="text-slate-400 text-[10px] uppercase block">Vá»‹ trÃ­ thá»±c táº­p:</span>
          ...
          <div class="space-y-2">
            ...
            <div class="p-4 bg-blue-50/70 border border-blue-100 rounded-2xl">
              <p class="font-extrabold text-blue-900 text-sm mb-1">{{ detail.deTai?.tenDeTai || 'ChÆ°a Ä‘Äƒng kÃ½ tÃªn Ä‘á» tÃ i' }}</p>
              <p class="text-slate-600 text-[11px] leading-relaxed">{{ detail.deTai?.moTa || 'MÃ´ táº£ ná»™i dung Ä‘á» tÃ i thá»±c táº­p táº¡i Ä‘Æ¡n vá»‹.' }}</p>
            </div>
          </div>
          ...
          <div class="space-y-3">
            <h4 class="font-bold text-slate-900 uppercase tracking-wider text-[11px] text-blue-900 border-b border-slate-100 pb-1">
              Báº£ng Ä‘iá»ƒm ÄÃ¡nh giÃ¡ Chi tiáº¿t
            </h4>
            <div class="space-y-2">
...
const statusLabels = {
  DangThucTap: 'Äang thá»±c táº­p',
  ChoGhiDanh: 'Chá» ghi danh',
  DungThucTap: 'Dá»«ng thá»±c táº­p',
...
const defaultGrades = [
  ...
  { maCotDiem: 2, tenCot: 'ÄÃ¡nh giÃ¡ tá»« Doanh nghiá»‡p (40%)', diemSo: 8.5 },
  { maCotDiem: 3, tenCot: 'ÄÃ¡nh giÃ¡ tá»« GV HÆ°á»›ng dáº«n (30%)', diemSo: 8.8 },
  { maCotDiem: 4, tenCot: 'BÃ¡o cÃ¡o / KhÃ³a luáº­n tá»‘t nghiá»‡p (20%)', diemSo: 8.5 }
];
